...
- **The v3 Lists API requires OR-wrapping-AND structure.** The root `filterBranch` must have `filterBranchType: OR` with at least one nested branch of `filterBranchType: AND`. Putting filters directly at the root OR level or using AND at the root throws: `"The root filter branch must have an 'OR' operator and type and must have at least one nested filter branch with an 'AND' operator and type."` Always wrap your AND filter groups inside the root OR, even for a single-branch list.
- **`IN_LIST` filter listIds are strings in v3.** Even though list IDs are integers everywhere else in HubSpot, the v3 create payload expects them as strings: `"listId": "611"` not `"listId": 611`.
- **`operationType` values are uppercase in v3.** Use `ENUMERATION`, `BOOL`, `TIMEPOINT` — not the lowercase values you'll see in v1 API read responses.
- **Deleted list references return null silently.** Querying a non-existent list ID via the v1 API returns HTTP 200 with all null fields rather than a 404. If a list appears to exist but returns empty data, verify it in the UI or try the v3 endpoint before assuming it's gone.
- **`TIME_POINT` operations require `"endpointBehavior": "EXCLUSIVE"`.** Omitting this field throws a validation error. Always include it on any `IS_BEFORE` or `IS_AFTER` date filter.
- **`IS_UNKNOWN` for date fields uses `operationType: ALL_PROPERTY`**, not `TIMEPOINT` or `ALLTYPES`. The full valid operationType list is: `ALL_PROPERTY`, `BOOL`, `CALENDAR_DATE`, `COMPARATIVE_DATE`, `DATE`, `DATETIME`, `ENUMERATION`, `MULTISTRING`, `NUMBER`, `RANGED_DATE`, `ROLLING_DATE_RANGE`, `STRING`, `TIME_POINT`, `TIME_RANGED`.
