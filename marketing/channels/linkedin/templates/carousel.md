...
> Asset-heavy post: 8–12 slides do the work, caption is short. Best for distilling a single argument into a sequence of scannable beats.

## When to use this type
- Wrapping a blog or webinar where you can extract a tight list ("5 metrics," "3 mistakes," "7-step framework")
- The argument has natural sub-units that each fit on one slide
...
## Future work
- Define max character count per slide once Canva templates lock layout
- Document the `Track:` / `Key:` / `Why:` callout convention (currently freeform)
- Decide whether the cover slide title must match the caption hook word-for-word
