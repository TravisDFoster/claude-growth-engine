...
- **What to do:** Run the command below. `pull_pipeline.py` fetches stage metadata + deal searches (new / closed / open / Foundations), per-rep **activity** counts (meetings/notes/tasks — calls read 0, emails scope-blocked), **demo-request form submissions** in the window (no owner filter; forms configured in `DEMO_FORMS` at the top of the script), and the structured **`feature_gaps` roll-up**, aggregates in Python, writes the JSON, then calls `render_report.py` to emit the HTML deterministically (reusing the reference's CSS verbatim — no sub-agent, no styling drift). **Two gotchas, both handled below:** `.env` must be exported with `set -a` (plain `source` won't reach the `uv` child); use absolute paths (the scripts resolve their own output paths from `__file__`).
  ...
  **Email activity (added 2026-06-22):** the activity roll-up now pulls **emails** alongside meetings/notes/tasks via `fetch_email_activity()` — per rep: **sent / received** (by `hs_email_direction`) plus **opens / clicks** engagement. It degrades gracefully: if the private-app token still lacks the email-read scope it records `status: "pending_scope"` and the report renders unchanged (email block shows a "pending scope" note). **To switch it on:** grant the HubSpot private app the `sales-email-read` / `crm.objects.emails.read` scope (Settings → Integrations → Private Apps → Scopes); the next run picks it up automatically — no code change. If opens/clicks aren't on the portal's email schema, it falls back to sent/received only (`status: "no_engagement"`).
...
- **Email activity (LIVE 2026-06-22):** scope granted and confirmed working — the activity roll-up now reports per-rep **sent / received / opens / clicks** from real data (`status: "ok"`). Property names confirmed: opens = `hs_email_open_count`, clicks = `hs_email_click_count` (both returned as float-formatted strings — see Learnings). Engagement covers tracked sends only (HubSpot sales-email/sequence sends); incoming emails and untracked sends contribute 0. Note: `hs_email_open_count` is *total* opens (one email opened 5× counts as 5), not unique-emails-opened. (Calls still read 0 — AEs don't log call engagements.)
...
- **2026-06-22 — Dropped calls from the activity section.** AEs never log call engagements (always read 0), so the column was dead weight. Activity now covers meetings/notes/tasks + the email engagement block. Removed from `ACTIVITY_TYPES` and the render's `types` list + footnotes.
- **2026-06-22 — Wired email activity into the roll-up (graceful-degrade ahead of the scope grant).** Added `fetch_email_activity()` to `pull_pipeline.py`: searches the `emails` engagement object (owner-filtered + windowed, same pattern as meetings/notes/tasks), splits **sent/received** off `hs_email_direction` (`INCOMING_EMAIL` = received), and sums **opens/clicks** off `hs_email_open_count`/`hs_email_click_count`. Renders as an "Email engagement" sub-block in section 04 (`_email_block()` in `render_report.py`). **Probed the live API first** — both `/objects/emails/search` and `/properties/emails` still 403 (`requiredGranularScopes: sales-email-read / crm.objects.emails.read`; properties need `connected-email-data-access`), so emails are genuinely unreadable until Travis grants the scope. Built the pull to degrade gracefully on that 403 (`status: pending_scope` → report renders with a "pending scope" note) and on a 400 from missing open/click props (`status: no_engagement` → sent/received only). Verified all three render states (ok / no_engagement / pending_scope) with synthetic data ahead of the grant. **Scope granted same day → went live.** First real run surfaced one bug: `hs_email_open_count`/`hs_email_click_count` come back as **float-formatted strings** (`"5.0"`), so the original `int(v)` raised `ValueError` and silently zeroed every open/click — fixed `_int()` to parse through `float()` first. Confirmed property names (opens `hs_email_open_count`, clicks `hs_email_click_count`; rates available too as `hs_email_open_rate`/`hs_email_click_rate`). Last-week sanity check: 43 sent / 24 received / 87 opens / 9 clicks across the trio. **Lesson:** HubSpot number properties are float strings even for integer counts — always parse via `float()`, and treat a flat run of 0s on a populated-looking property as a parse bug, not "no data."
